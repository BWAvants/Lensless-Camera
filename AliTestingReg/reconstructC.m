...
global Phi_rec_leftC Phi_rec_rightC channelIndex sensorSize screenPatchSize waveletLevel QMF

targetImages = 'realImages';
saveFigs = false; test_indices = [];%[13 21 127];% 125 127] ;%[1 3 5] [7 149 159]
...
maskType = '255M030C';
switch maskType
    case {'16Mseq','08Mseq','031M100','127M060','255M030','255M030C'}
...
for i = 1:length(invMode)
    ...
    switch invMode{i}
        case {'svd', 'svdTV','TV-unlocbox','FTVd'}
            ...
            switch invMode{i}
                case 'svd'
                    T = round(screenPatchSize/4); lambda = 4;
                    ...
                    iPhi_rec_svd = @(z) vec(trim(im2col(iPhi_rec_left(iPhi_rec_right(z)),[screenPatchSize screenPatchSize],'distinct')));        
%                 case 'svdTV'
...
switch targetImages
    ...
    case {'testImages', 'simulatedImages'}
        ...
        sensorTest = double(tmp_mat.sensorTest_highres);
        sceneTest = double(tmp_mat.highresImages);
    case 'testVideos'
        ...
        for channelIndex = 1:numOfChannels
            ...
            switch invMode{i}
                ...
                case 'Wave+FastaLasso'
                    scene_test_rec(sceneChannelRange,tst,i) = vec(IWT2_PO(reshape(fasta_lasso(op_Phi_rec_WaveF,op_Phi_rec_WaveB,sensor_test(sensorChannelRange,tst),muFastaLasso, zeros(screenPatchSize^2,1), optsFastaLasso),screenPatchSize,screenPatchSize),waveletLevel,QMF));
                case 'Wave+FastaLSQR'
        ...
        if ~saveFigs
            ...
            for tst = 1:length(test_indices)
                for i = 1:length(invMode)
                    ...
                    imshow(thisImageNormalized);title(invMode{i});
                    k = k + 1;
                end
            end
        else
