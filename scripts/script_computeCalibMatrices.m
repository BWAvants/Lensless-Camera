...
fprintf('reading calibration images ... ');
...
% use white image to compute and normalize white balance!
white_balance_correction = ones(1,numOfChannels);
if numOfChannels > 1
    sum_img_rgb = mean(mean(reshape((sensorImage_leftC(:,1)+sensorImage_rightC(:,1))/2, sensorSize(1), sensorSize(2),sensorSize(3)),1),2);
    white_balance_correction(1) = sum_img_rgb(1)/sum_img_rgb(2);
    white_balance_correction(3) = sum_img_rgb(3)/sum_img_rgb(2);
    
    
    % adjusting white balance in calibration data
    % 2x more weight assigned to the green channel vs red/blue while averaging
    % y - sensor image, x - screen image
    % y_sum = alpha_R Phi x_R + 2*alpha_G Phi x_G + alpha_B Phi x_B
    %
    % adjust white balance
    % sensorImage_leftC = bsxfun(@times, reshape(sensorImage_leftC, sensorSize(1), sensorSize(2),sensorSize(3),[]),permute([1 2 1]./white_balance_correction,[3 1 2]));
    % sensorImage_rightC = bsxfun(@times, reshape(sensorImage_rightC, sensorSize(1), sensorSize(2),sensorSize(3),[]),permute([1 2 1]./white_balance_correction,[3 1 2]));
    %
    sensorImage_leftC = reshape(sensorImage_leftC, sensorSize(1), sensorSize(2),sensorSize(3),[]);
    sensorImage_rightC = reshape(sensorImage_rightC, sensorSize(1), sensorSize(2),sensorSize(3),[]);
    
    % average RGB channels into one.
    sensorImage_left = reshape(mean(sensorImage_leftC,3),prod(sensorSize(1:2)),[]);
    sensorImage_right = reshape(mean(sensorImage_rightC,3),prod(sensorSize(1:2)),[]);
    sensorImage_leftC = []; sensorImage_rightC = [];
else
    sensorImage_left = reshape(sensorImage_leftC,prod(sensorSize(1:2)),[]);
    sensorImage_right = reshape(sensorImage_rightC,prod(sensorSize(1:2)),[]);
    sensorImage_leftC = []; sensorImage_rightC = [];
end
