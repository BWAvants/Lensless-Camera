...
msg1 = sprintf('computeCalibC separable: calibration-%s, screenPatchSize=%d, maskType=%s, downSampling=%02d (%s)',...
...
switch invMode
    case 'svd'
        ...
        iPhi_rec_left = @(z) reshape(Vl(:,1:T)*(iSl(1:T,1:T)*(Ul(:,1:T)'*reshape(z,sensorSize(1),[]))),[],sensorSize(2),sensorSize(3));
        iPhi_rec_right = @(z) permute(reshape([reshape(permute(z,[1 3 2]),[],sensorSize(2))*Vr(:,1:T)*iSr(1:T,1:T)*Ur(:,1:T)'],[],sensorSize(3),screenPatchSize),[1 3 2]);
        % iPhi_rec_right = @(z) reshape((Ur(:,1:T)*(iSr(1:T,1:T)*(Vr(:,1:T)'*reshape(permute(vec2rgb(z),[2 1 3]),sensorSize(2),[]))))
        iPhi_rec = @(z) vec(iPhi_rec_right(iPhi_rec_left(z)));
        
        op_Phi_rec = @(z) op_separableSystem(z,1, Phi_rec_left, Phi_rec_right, screenPatchSize,sensorSize);
...
if saveFigs
    ...
    test_indices = [15 17 21 125:2:129];
end
% test_indices = find(sum(abs(sensorTest),1)>0);
